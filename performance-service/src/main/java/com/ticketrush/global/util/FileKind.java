 ...
 *
 * <p><b>다만 "비어 있는 파트"의 취급은 파트마다 갈린다.</b> 교체는 빈 파트를 보내지 않은 것으로 보고 넘기고(#637 — 보낸 파트만 교체한다는 계약을 지키려면
 * 그래야 한다), 등록의 {@code model3d}도 #650부터 선택이라 같은 정의를 따른다. 반면 등록의 {@code mainImage}는 여전히 필수라 여기 {@link
 * #validate}가 {@code FILE_EMPTY}로 거절한다. 등록의 갤러리도 같은 관대함이 필요해지면 그때 두 경로를 다시 맞춘다.
 *
