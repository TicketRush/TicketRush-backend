...
**범위를 나타낼 때 물결표(`~`)가 아니라 하이픈(`-`)을 씁니다.**

GitHub 마크다운은 `~텍스트~`를 **취소선**으로 렌더링합니다. 한 문단에 물결표가 둘 이상 있으면 **그 사이가 통째로 줄이 그어져** 읽을 수 없게 됩니다.

| | 예시 |
|---|---|
| ❌ | `피크(00:22:19~00:27:19)에는 … 피크가 끝난 00:28~00:32 에` → 가운데가 취소선 |
| ✅ | `피크(00:22:19-00:27:19)에는 … 피크가 끝난 00:28-00:32 에` |

* **적용 대상:** 이슈 본문·코멘트, PR 본문, 저장소의 모든 마크다운 문서(리포트·런북·ADR·스킬).
* **시각·수량·기간 모두 해당:** `5-6분`, `40-70%`, `90-150ms`, `10-20개 파일`, `2026-07-27T14:38:24Z - 15:05:05Z`.
* 인라인 코드나 코드블록 안(`` `00:22~00:27` ``)은 렌더링되지 않아 안전하지만, **본문에서는 일관되게 `-`를 씁니다.**
