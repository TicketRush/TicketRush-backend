...
* **리뷰어는 컨벤션 표대로 자동 지정**(작성자→리뷰어), **assignee는 본인**(`@me`). **`✅ 테스트` 항목은 자동으로 채우지 않고 비워 둡니다**(실제 테스트는 사람이 작성 — 추측 작성으로 인한 오인 방지).
...
* 수정 건은 GitHub 코멘트에 **답글**(승인 후 전송), 거절 건은 사유 답글. resolve는 수동.
* 검증(`/test`·spotlessCheck) 후 묶음 1커밋을 `/commit`에 위임.
