...
* **리뷰어는 컨벤션 표대로 자동 지정**(작성자→리뷰어), **assignee는 본인**(`@me`). 단 **코드 리뷰는 선택(권장)이며 머지 필수 조건이 아닙니다**(approve 없이도 머지 가능). **`✅ 테스트` 항목은 자동으로 채우지 않고 비워 둡니다**(실제 테스트는 사람이 작성 — 추측 작성으로 인한 오인 방지).
...
* PR 업로드 후 리뷰 코멘트가 달리면(보통 비동기로 나중에 도착) **`/apply-review`로 대응**합니다(사이클 10단계).
