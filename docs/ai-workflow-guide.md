...
### 4.9. `/wrap-up` — 세션 작업 요약 + 슬랙 전송

이번 세션에서 한 작업을 **고정 형식으로 요약**해 보여주고, 확인 후 **슬랙 채널로 전송**합니다. 메인 컨텍스트에서 실행(대화 맥락 활용)하며, 코드·커밋·브랜치 변경 없이 읽기 + 요약·전송만 합니다. `/resume`(세션 재개)과 짝을 이루는 세션 마무리 커맨드입니다.

* 요약은 한국어 4개 섹션으로 고정: `## 한 일` / `## 주요 결정·근거` / `## 변경/커밋` / `## 다음 할 일·미해결`. 빈 섹션은 `- 없음`.
* 슬랙 웹훅 URL은 `$env:SLACK_WRAP_WEBHOOK`(gitignore된 `settings.local.json`의 `env`)에서만 읽습니다. 비어 있으면 전송을 스킵하고 안내하며, 실패 시 보고 후 멈춥니다(재시도 안 함).
* GitHub 이슈/PR은 건드리지 않습니다(그건 `/commit`·`/pr` 담당).
* ⚠️ `/pr`과 동일한 draft-승인 철학: 먼저 화면에 초안을 표시한 뒤 확인받고 전송합니다. `바로전송` 지시 시 확인을 생략합니다.
* 예시: `/wrap-up` 또는 `/wrap-up 바로전송`
