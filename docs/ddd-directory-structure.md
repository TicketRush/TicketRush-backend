...
> **`boundedcontext/` 아래의 단위는 서비스가 아니라 도메인입니다.** 대부분의 서비스는 서비스명과 도메인명이 같아 컨텍스트를 하나만 갖지만, 한 서비스가 컨텍스트를 여럿 가질 수 있습니다. 첫 사례와 그 판단 기준은 [ADR 0013](adr/0013-banner-as-separate-context-inside-performance-service.md) 참고.
...
* **공통 모듈 규칙**: 모듈 공통으로 사용해야 하는 코드는 `common`에 적습니다.
* `shared` 디렉토리는 `common` 모듈에서만 사용합니다.
