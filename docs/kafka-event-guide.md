...
- **409를 "이미 처리된 중복"으로 삼키려면 피호출 측이 멱등 성공을 2xx로 답하는 것이 전제다.** 그 전제 없이 409를 warn으로 뭉개면 정상 중복과 정합성 위반이 같은 코드로 도착해 후자가 로그 한 줄에 묻힌다. 예) seat `POST /internal/seat/sold`는 **이미 같은 예매로 SOLD면 200**을 반환하고(#489), 409(`SEAT_409_003`)는 "그 좌석이 이 예매의 것이 아님"이라는 치명적 실패에만 쓴다. 그래서 booking `PaymentConfirmedEventListener`는 이 409를 `[CRITICAL]` + `SeatConfirmFailedEvent` 발행으로 처리하고, 4xx 전체가 가시화 대상이 된다. 이 규약이 없던 시절 실측에서 **과금 완료 + 좌석 미확정**이 "중복 수신" warn 한 줄로 사라졌다(#345 §6.1).
