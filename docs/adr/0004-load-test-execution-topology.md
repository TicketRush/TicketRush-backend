...
- (한계) 요청 경로는 여전히 가정용 업로드를 쓰므로 초고 VU에서는 업로드 대역이 상한이 될 수 있다. 이 규모를 넘어서야 하면 AWS 내부에 부하 생성기를 두거나 관리형 부하 도구(k6 Cloud 등)를 별도 ADR로 검토한다. → **[ADR 0010](0010-in-aws-load-generator-for-ten-thousand-vu.md)이 1만 VU 회차에 한해 이 조항을 따라 생성기를 AWS 안으로 옮겼다.** 그 아래 규모는 이 ADR이 그대로 유효하다.
