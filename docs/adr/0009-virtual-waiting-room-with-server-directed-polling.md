...
**12는 고정 상수라는 점을 함께 기록한다.** 부하는 변하는데 값은 변하지 않으므로 12는 최악 계단에 맞춘 보수적 값이고 평시에는 그만큼 손해를 본다. 하류 신호(좌석맵 p95 · seat-service CPU · Hikari `pending`) 기반의 적응형 유입 제어가 그 손해를 없애는 경로이고, 아래 후속 작업에 둔다.
...
- **입장 허용량의 적응형 제어** — 현재 `admit`은 고정 상수라 최악 계단에 맞춰 잡히고 평시에는 그만큼 손해를 본다(§3.5). 하류 신호(좌석맵 p95 · seat-service CPU · Hikari `pending`)를 읽어 허용량을 올렸다 내렸다 하면 그 손해가 없어진다. **#613으로 천장을 올린 뒤에 볼 항목이다** — 팬아웃 비용이 그대로면 적응형으로 바꿔도 수렴점이 12 근처다.
