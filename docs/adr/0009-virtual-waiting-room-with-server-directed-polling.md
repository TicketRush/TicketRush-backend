...
**예매 API는 유효 입장 토큰만 통과시킨다.** 그 결과 예매 경로의 부하가 유입 규모와 무관하게 "입장 허용량"으로 고정된다 — 이것이 이 설계의 전부다. **허용량의 근거는 §3.5에 있다**(#555 계단 실측, 2026-08-14).

> ⚠️ 이 문장은 원래 *"허용량은 위 실측(booking 단독 258 RPS에서 CPU 99.87%)의 아래로 잡는다"*였다. **그 258 RPS를 허용량의 근거로 쓰면 안 된다** — 예매 처리량이 아니라 반려된 요청의 RPS다(§3.5). 위 표의 #344 행은 "유입을 그대로 받으면 CPU가 먼저 죽는다"는 사실의 기록으로만 읽는다.
...
**이 이슈가 주 성과 지표로 둔 "1만 명 소화 시간"에서도 같은 결론이 나온다.** 완주를 **좌석맵 통과 + 예매 성공**으로 정의해 환산하면 admit 12가 21분 32초-24분 38초인데 **16은 32분 24초 · 28분 53초로 두 회차 모두 더 느리다** — 허용량을 올렸는데 완주 처리율이 떨어진다. 20은 18분 59초-48분 46초로 **편차가 2.57배**이고(12는 1.14배) 최악 회차에서는 예매 자체가 82.77%로 무너졌다. 티켓 오픈은 재시도가 없는 일회성 이벤트라 기대값이 아니라 최악값으로 설계한다.

> 예매 성공만 세면 admit 20이 더 빨라 보인다. 그 수치에는 **좌석맵을 못 본 사람이 완주자로 섞여 있다** — 부하 여정이 좌석맵 실패에도 예매를 진행하기 때문이다(아래 한계). 리포트 §3.3이 두 정의를 나란히 놓고 재계산했다.

**12는 전역 상수라는 점을 함께 기록한다.** 이 값은 공연별이 아니라 게이트웨이 전역이고, **"공연 하나만 열려 있다"를 가정한 값이다** — 이 회차가 공연 하나로만 측정했기 때문이다. 두 공연이 동시에 열리면 같은 2 vCPU를 나눠 쓰므로 **12도 안전하지 않다.** 좌석 수가 다르면 팬아웃 부하도 다르다(#590이 공연별 총 좌석 수를 120-10,000으로 열었다).

> **"고정값이라 평시에 손해를 본다"는 근거는 이 도메인에서 약하다.** 대기열이 켜져 있는 시간은 오픈 직후 몇 분에서 수십 분이고 그 구간에 한산한 때가 없다 — 적응형 제어의 교과서적 이득("평시엔 높게")이 여기서는 거의 발생하지 않는다. **실질적인 근거는 동시 오픈과 공연 규모 차이다.**

적응형 유입 제어(하류 신호를 읽어 허용량을 조절)가 그 경로이고 아래 후속 작업에 둔다. **다만 지금은 만들 수 없다** — 게이트웨이에 하류 신호를 읽는 경로가 아예 없기 때문이다(후속 항목 참조).
...
**주 지표로도 같은 크기가 나온다** — 같은 `admit 16`의 1만 명 완주 환산이 **32분 24초 → 18분 32초**다. 팬아웃이 소화 시간에서 **1.75배**를 먹고 있고, 구독자를 뺀 16은 admit 20을 포함해 전 계단에서 가장 빠른 축에 든다. **막고 있는 것은 `admit`이 아니다.**

그리고 **구독자 100명은 과대가 아니라 과소 가정이다.** 실제 동시 구독자 ≈ `admit × 좌석 선택 화면 체류시간`이므로 `admit 12` + 체류 30초만 해도 360명이고 [#403](https://github.com/TicketRush/TicketRush-backend/issues/403)은 600명까지 검증했다. 팬아웃 비용이 `admit × 구독자` = **`admit²`로 커진다는 뜻이라, 이 사실 자체가 상위 계단을 기각하는 근거이기도 하다** — 실제 오픈은 이 실측보다 나쁘고, 그 격차가 계단이 높을수록 벌어진다.
...
- **부하 여정이 좌석맵 실패를 예매로 전파하지 않는다.** 여정은 좌석맵 GET 결과와 무관하게 예매를 호출하고 좌석이 미리 배정돼 있다. 그래서 예매 성공 기준 지표(예매 서버 RPS · 소화 시간)가 좌석맵을 못 본 사람을 완주자로 세어 **상위 계단 쪽으로 편향된다.** 위 결론은 완주자 기준으로 다시 세어 보정한 것이다(리포트 §3.3).
- **계단을 admit 오름차순으로 돌지 않았고 계단 사이 서비스 재시작이 없다.** 두 시리즈 모두 16이 20보다 뒤에 돌았다. admit 20의 좌석맵 성공률이 38.14% ↔ 73.26%로 재현되지 않은 것에 순서 효과를 배제할 수 없다 — 그래서 이 ADR은 20의 처리량 우위를 근거로 쓰지 않는다.
...
- [#613](https://github.com/TicketRush/TicketRush-backend/issues/613) **좌석 SSE 팬아웃 비용 절감** — #555가 특정한 무릎의 원인이다. 이것을 낮추기 전에는 `admit`을 올릴 수 없다.
  ...
  - **이득의 상한은 이미 측정돼 있다.** 같은 `admit 16`에서 구독자만 빼자 1만 명 완주 환산이 **32분 24초 → 18분 32초(1.75배)**였다(§3.5). 팬아웃 비용을 0으로 만들었을 때의 상한이고, 실제 이득은 직렬화 비용과 소켓 쓰기 비용의 비율에 달렸다 — **그 비율은 재 봐야 안다.**
  - 구독자 수 가정이 `admit²` 비용으로 이어진다는 근거는 §3.5에 있다.
- **입장 허용량의 적응형 제어** — 현재 `admit`은 게이트웨이 전역 상수이고 **공연 하나만 열린다고 가정한 값이다**(§3.5). 동시 오픈이나 좌석 수 차이에서 12가 틀린 값이 된다. 하류 신호를 읽어 허용량을 조절하면 그 가정이 사라진다.
  - **선행 조건이 둘이고, 둘 다 아직 없다.** ① **게이트웨이에 하류 신호를 읽는 경로가 없다** — `gateway-service`의 대기열 패키지에는 seat-service로 가는 클라이언트도 Prometheus 질의도 없고, `WaitingRoomPolicy`의 입력은 전부 Redis와 설정값이다. 지금 제어 루프를 만들면 **#555가 특정한 병목(seat-service)을 보지 못한다.** 그 신호를 어떻게 전달할지가 별도 설계 결정이다. ② 팬아웃 비용이 그대로면 수렴점이 12 근처라 이득이 작다.
  - 그래서 **#613 다음 순서다.** `queue_status_unavailable`이 포화의 원인이 아니라 **결과**로 뜬 것(§3.5 · 리포트 §5.1)이 ①의 증거다 — 게이트웨이의 자기 지표는 seat-service 붕괴를 늦게 본다.
  - 공연 규모 차이만이라면 적응형이 아니라 **공연별 파라미터**가 더 싼 해법이다. 먼저 검토한다.
