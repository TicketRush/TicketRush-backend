...
| 시점 | `process_resident_memory_bytes` | 상한 대비 | `prometheus_tsdb_head_series` | `scrape_duration_seconds` (max) |
|---|---:|---:|---:|---:|
| **#402 부하 중** (2026-07-26T10:10:00Z, 피크 180 req/s + remote-write 수신) | 121.1 MiB | 31.5% | 6,618 | 9.9 ms (mvc) / 3.7 ms (gateway) |
| 유휴 (2026-07-26 11:20 KST, 스택 기동 13분 후) | 146.7 MiB | 38.2% | 7,109 | 11.1 ms |
...
| `scrape_duration_seconds` (max) | 9.9 ms (mvc) / 3.7 ms (gw) | 20.8 / 15.3 ms | **17.5 / 9.7 ms** |
...
- **`scrape_duration`이 약 2배가 됐지만 절대값이 무의미하게 작다.** 워밍업 후 mvc 9.9 → **17.5 ms**, gateway 3.7 → **9.7 ms**로 `scrape_interval` 15s의 0.12% 수준이다. 응답 페이로드에 버킷 라인이 붙은 대가로 읽는다. **배포 직후값(20.8 / 15.3 ms)은 JIT·클래스 로딩이 섞여 과대평가였다** — 20분 뒤 재측정에서 내려갔다. #402 §6.1이 같은 함정을 CPU·p95에서 기록했고, 스크랩 시간도 같은 성질임을 확인했다.
