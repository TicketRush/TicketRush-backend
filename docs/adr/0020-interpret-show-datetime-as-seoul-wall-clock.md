...
- **(#671) `booking_open_at` 응답 형식 변경은 계약 변경이지만 배포 순서는 자유롭다.** 바뀌는 것은 존 표시뿐이고(`2027-08-01 20:00:00` → `2027-08-01T20:00:00+09:00`) 숫자는 그대로라, `new Date`·ISO 파서 쪽은 오히려 정확해진다 — 기존 무시간대 형식이야말로 브라우저마다 갈렸다. 깨지는 경로는 문자열을 직접 자르는 쪽뿐이고, 그중 실제로 막히는 것은 어드민 수정 화면이다: 상세 응답을 폼에 되돌려 저장하면 요청 DTO(`yyyy-MM-dd HH:mm:ss`)가 `T`를 거부해 400이 된다. **조용한 오염이 아니라 거부**이므로 그 화면만 프론트에서 맞추면 되고, 백엔드를 먼저 배포해도 된다. 롤백하면 무시간대 형식이 돌아오므로 프론트가 두 형식을 모두 파싱하면 순서 자체가 논점이 아니게 된다. 요청 형식은 바뀌지 않는다.
