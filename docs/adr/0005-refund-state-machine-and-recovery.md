...
- (확장 · #668) **환불 마감(D-7)을 서버에서 강제한다.** 공연 7일 전까지만 환불을 허용하는 제한이 프론트 `isRefundableBooking`에만 있어, `DELETE /api/v1/booking/{bookingNumber}`를 직접 호출하면 공연 당일에도 환불이 성사되고 PG 환불까지 진행됐다. 인가 우회가 아니라 비즈니스 규칙 우회지만, 금전이 오가는 경로이므로 신뢰 경계인 서버에서 다시 검증한다. 가드는 `BookingValidateRefundDeadlineUseCase`이며 판정 기준은 `RefundDeadlinePolicy`가 소유하고, 위반은 `BOOKING_REFUND_DEADLINE_PASSED`(`BOOKING_409_007`)로 거절한다.

  확정한 정책 경계는 이렇다.

  - **기준 시각은 공연 시작 일시다.** `show_date` + `show_time`을 **Asia/Seoul 벽시계**로 해석한 순간에서 7일을 뻐다. 주입되는 `Clock`은 UTC(`ClockConfig`)이므로 `LocalDateTime.now()`로 비교하면 UTC 컨테이너에서 9시간 어긋난다 — ADR [20](0020-interpret-show-datetime-as-seoul-wall-clock.md)(#653)이 `bookingOpenAt`에서 세운 규율을 그대로 따른다.
  - (알려진 한계) **목록 화면에서는 프론트와 판정이 최대 하루 폭으로 엇갈릴 수 있다.** 프론트는 공연 시각이 있으면 시각 기준으로, 없으면 달력 일수로 판정한다. 그런데 예매 **목록** 응답(`BookingMySummaryResponse`)은 `performanceDate`만 내리고 시각을 내리지 않아(상세 응답 `BookingDetailResponse`는 내린다), 목록 화면의 버튼은 달력 일수 폴백을 탄다. 예: 공연 7/20 19:30, 현재 7/13 21:00 KST → 달력로는 7일이라 버튼은 활성이지만 서버는 잔여 6일 22.5시간이라 409를 낸다. 경계를 `>=`로 맞춘 목적(버튼 활성 ↔ 409 불일치 제거)이 목록 화면에서는 완전히 달성되지 않는다. 서버가 더 엄격한 방향이라 정책이 뚫리지는 않지만, 근본 해소는 목록 응답에 `show_time`을 추가하는 것이다 — 응답 계약 변경이라 #668 범위 밖으로 둠.
  - **경계(정확히 7일)는 허용한다.** 프론트가 `>= 7`로 판정하므로 서버에서 `>`를 쓰면 경계에서 "버튼은 활성인데 API는 409"인 불일치가 생긴다. 두 판정은 같은 방향으로 움직여야 한다.
  - **booking 쪽에서 가드가 걸리는 곳은 사용자 `CONFIRMED` 환불 경로 하나뿐이다.** (결제 취소 API 는 아래 항목에서 따로 닫는다.) 기존 입장 가드(`BookingValidateTicketNotUsedUseCase`)와 클래스를 나눈 이유가 이것이다 — 그쪽 `executeForAdmin`은 관리자 환불과 재환불이 함께 쓰는데, 여기서는 두 경로 모두 D-7을 적용하면 안 된다.
    - **PENDING 즉시 취소** — 결제 전 이탈이라 환불이 아니다. 공연일과 무관하게 계속 허용한다.
    - **관리자 환불(`POST /{n}/refund`)** — 정책 예외를 처리하려고 있는 CS 창구다. 막으면 예외 대응 수단 자체가 사라진다.
    - **관리자 재환불(`POST /{n}/refund-retry`) — 절대 걸지 않는다.** 이 경로는 `REFUNDING` 고착(#397) 복구를 겸한다. 막으면 고착을 빠져나올 유일한 수단이 사라져 이 ADR이 제거한 흡수 상태가 되살아난다. 게다가 고착은 **공연이 지난 뒤에야 발견되는 경우가 주 대상**이라, D-7을 걸면 정확히 그 건들이 복구 불가가 된다. 위 #399가 같은 이유로 입장 가드를 이 경로에서 풀어둔 것과 동일한 판단이다.
  - **검사 대상은 환불이 성사될 수 있는 예매뿐이다 — `CONFIRMED`와 `REFUNDING`.** 입장 가드와 같은 술어를 쓴다. `REFUNDING`을 그냥 통과시키고 싶은 유혹이 있었지만(뒤이은 `requestRefund()`가 어차피 409로 거절하므로 왜복이 낭비처럼 보인다), **그러면 복원 창이 열린 채로 남는다.** `recordRefundFailure`(#391)가 `REFUNDING`을 `CONFIRMED`로 되돌리므로, 가드가 `REFUNDING`을 관측해 early return 한 뒤 취소 트랜잭션이 `CONFIRMED`를 읽으면 `requestRefund()`가 성공한다 — 그 건은 **마감 판정을 한 번도 거치지 않고** 환불을 개시한다. "마감은 상태가 아니라 공연 일시에만 의존하니 결과가 같다"는 논증은 **판정을 수행했을 때만** 참이다. `CONFIRMED`로 복원될 수 있는 상태가 `REFUNDING`뿐이므로 이 둘만 보면 창이 닫힌다. `PENDING`은 위 이유로 대상이 아니고, `CANCELED`·`EXPIRED`·`REFUNDED`는 `CONFIRMED`로 돌아오는 경로가 없어 조회 없이 통과시킨다.
  - (트레이드오프) **performance-service 조회 실패 시 환불이 막힌다(fail-closed).** `PerformanceRestClient`는 예매 목록 조회를 위해 의도적으로 fail-open이라(#560 부분 응답 정책) 실패를 `Optional.empty()`로 수렴시킨다. 그 빈 결과를 "제한 없음"으로 읽으면 performance-service 장애가 곧 D-7 무력화가 되므로, **호출자인 가드가 빈 결과를 차단으로 해석**한다(클라이언트는 고치지 않는다 — 고치면 목록 조회가 함께 죽는다). 공연 일시를 확정할 수 없는 경우는 모두 `BOOKING_PERFORMANCE_COMMUNICATION_FAILED`(`BOOKING_503_002`)로 수렴시킨다. `show_date`·`show_time`은 `Performance`에서 `nullable = false`라 정상 응답이면 비어 있을 수 없고, 비어 있다면 스키마 변경이나 라우팅 오설정이므로 낙관하지 않는다 — 위 "환불을 허용하는 분기는 최소한으로 좁힌다"와 같은 규율이다.
  - **조회는 `@Transactional` 밖에서 한다.** performance-service 왕복이 트랜잭션 안에 들어가면 다운스트림 지연이 booking의 DB 커넥션 풀을 물어 예매 경로 전체를 마비시킨다 — 입장 가드와 같은 이유다. 잔여 위험도 동일하게 DB 커넥션이 아니라 톰캣 스레드다.
  - (트레이드오프) **check-then-act 창이 남는다.** 가드 통과와 `requestRefund()` 커밋 사이(수 ms)는 입장 가드와 같은 한계다. 다만 마감은 시간에만 의존해 단조롭게 지나가므로, 이 창으로 통과할 수 있는 것은 경계를 막 넘긴 요청뿐이다. 분산 락은 도입하지 않았다.
  - (알려진 한계) **`7`이 프론트와 백엔드 두 곳에 하드코딩된다.** 설정으로 빼면 한쪽만 바뀌어 "버튼은 보이는데 409"가 조용히 생기므로, 눈에 띄게 상수로 두고 정책 변경 시 두 레포를 함께 고치는 쪽을 택했다.
  - (알려진 한계) **삭제된 공연의 예매는 셀프 환불이 영구히 막힌다.** `PerformanceDeleteUseCase`는 SOLD 좌석 가드가 아직 TODO(#87)라 CONFIRMED 예매가 있는 공연도 소프트 삭제된다. 그러면 조회가 상시 404라 fail-closed가 매번 걸려 `BOOKING_503_002`가 영구화된다 — 메시지는 "잠시 후 다시 시도"를 안내하지만 재시도로 풀리지 않는다. `getPerformance`가 4xx와 5xx를 구분 없이 `Optional.empty()`로 수렴시키기 때문인데, 구분한다 해도 "삭제된 공연의 환불을 허용할지"는 별도 정책 결정이라 여기서 정하지 않았다. 탈출구는 **관리자 환불**이다 — 그 경로는 D-7 면제이고 공연 조회도 하지 않으므로 그대로 동작한다. 근본 해소는 #87이다.
  - **결제 취소 API(`POST /api/v1/payment/{paymentId}/cancel`)에도 같은 마감을 건다.** 이 경로는 booking 의 취소 API 를 거치지 않고 PG 를 직접 치므로, booking 에만 가드를 두면 사용자가 엔드포인트를 바꿔 부르는 것만으로 정책이 통째로 우회된다. `PaymentCancelUseCase` 가 PG 취소 **앞**에서 fail-closed 로 막고, 마감 위반은 `PAYMENT_CANCEL_NOT_ALLOWED_REFUND_DEADLINE`(`PAYMENT_409_004`)이다. #416 이 같은 이유로 입장 가드를 이 경로에 복제한 것과 같은 판단이며, 순서(PG 앞)도 같은 이유다 — 뒤에 두면 환불은 나가고 좌석만 SOLD 로 남아 수동 DML 외에 풀 수단이 없다.
    - **판정 주체는 booking 이다.** payment 가 공연을 직접 조회해 계산하면 `7` 이 프론트·booking·payment 세 곳에 하드코딩되어 가지각각 움직일 수 있다. booking 의 internal 조회(`GET /api/v1/internal/booking/{bookingId}`)에 `withRefundDeadline=true` 를 붙이면 `refund_allowed` 에 판정이 실려 오고, payment 는 결과만 쓴다.
    - **그 파라미터를 기본값 `false` 로 둔 이유.** 판정에는 booking → performance 왕복이 한 번 더 든다. 같은 엔드포인트를 **ticket-service 의 입장 검증**과 payment 의 결제 확정·만료 보상도 쓰는데, 그 셋은 마감을 필요로 하지 않으면서 지연에 민감하다(특히 입장은 공연장 게이트다). 기본값으로 켜면 그 경로들이 전부 비용을 문다. 요청 URI 도 `true` 일 때만 파라미터가 붙어 기존 호출의 형태가 바뀌지 않는다.
    - **`refund_allowed` 가 null 이면 차단이다.** `withRefundDeadline=true` 로 요청했는데 비었다면 배포 순서 역전이나 필드명 변경이라는 뜻이고(`@JsonIgnoreProperties` 라 조용히 null 이 된다), 통과시키면 그 구간에 정책이 사일런트로 무력화된다 — `bookingNumber`(#608)에 적용한 규율과 같다. booking 이 공연 일시를 확정하지 못하면 애초에 `false` 가 아니라 503 으로 끝나므로, fail-closed 가 두 서비스에 걸쳐 유지된다.
    - (남는 구멍) 관리자 환불·재환불은 여전히 면제다. 그쪽은 의도된 예외이고 ADMIN 롤로 격리돼 있다.
  - **공연 취소(`PerformanceStatus.CANCELED`)는 D-7 예외로 두지 않는다.** 공연이 D-7 안쪽에서 취소되면 피해자는 셀프 환불 대신 **관리자 환불**로 처리한다. 애초에 공연 취소 시 자동 환불 플로우가 없어 그 경로가 이미 사실상의 담당 창구이고, 여기에 예외를 만들면 `PerformanceInfoResponse` 에 상태를 실어 booking 이 공연 상태 정책까지 판정하게 된다. 공연 취소 환불을 자동화할 때 그 설계와 함께 다루는 편이 맞다.
  - **프론트 제한은 그대로 둔다.** 서버 가드가 신뢰 경계이고, 프론트 판정은 버튼을 미리 비활성화해 실패할 요청을 보내지 않게 하는 UX 장치로 남는다.
