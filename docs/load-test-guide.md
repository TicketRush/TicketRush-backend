...
k6 (로컬 loadtest 컨테이너) ──HTTP────────────> AWS 배포 앱 (공인 게이트웨이 :8080)
        │                                              ▲
        └─ remote-write ─> EC2 Prometheus :9090 ──scrape─┘   (k6_* + ticketrush_* 한 TSDB)
                                    │
                                    └──> EC2 Grafana :3000
```

- 관측 스택(Prometheus·Grafana)은 **대상과 같은 EC2**에 있다. 로컬 k6의 remote-write 대상만 EC2로 돌리면 **k6 클라이언트 시계열(`k6_*`)과 앱 계측(`ticketrush_*`)이 같은 TSDB에 모인다.** 대시보드 JSON은 datasource uid `prometheus`를 참조하므로 수정할 것이 없다.
...
### 7.1 사전 조건 (AWS)
- 부하 대상 앱을 AWS에 배포하고 게이트웨이 공인 엔드포인트(`http://<EC2_IP>:8080`)를 확보한다. 배포 구성은 [ADR 0005](adr/0005-aws-deployment-topology.md) 참고.
- 관측 스택이 EC2에서 기동 중이고, Prometheus 9090 포트가 **본인 공인 IP에 대해서만** 열려 있어야 한다(remote-write 수신에는 인증이 없다).
- `seat-layouts`(read)는 **인증 불필요** → 배포 프로파일과 무관하게 바로 가능.
- `booking-create`(write)는 `DevToken(/api/v1/dev/auth/token)`을 사용하는데 이는 **`local`/`dev` 프로파일에서만 노출**된다. → AWS 배포본을 **`SPRING_PROFILES_ACTIVE=prod,dev`**로 띄우지 않으면 write 시나리오는 **인증 실패**한다.
  > `application-dev.yml`은 어느 모듈에도 없다. `dev` 단독으로 띄우면 datasource 설정이 없어 부팅 자체가 실패한다. `prod`로 접속 정보를 주입받으면서 `dev`만 추가 활성화해야 한다.
- 시딩은 3장과 동일하게 **AWS의 부하테스트용 DB**에 대해 수행한다(운영 DB 금지).

### 7.2 k6 실행 (로컬)
4장과 동일하되 `BASE_URL`과 `K6_PROMETHEUS_RW_SERVER_URL`을 AWS로 덮는다. 로컬 모니터링 스택은 띄우지 않는다.
...
docker compose run --rm --no-deps \
  -e K6_PROMETHEUS_RW_SERVER_URL=http://<EC2_IP>:9090/api/v1/write \
  k6 run \
  -e BASE_URL=http://<EC2_IP>:8080 \
  -e PERF_ID=1 -e VUS=50 \
  /scripts/scenarios/seat-layouts.js

# (a) 예매 생성 — AWS가 prod,dev 프로파일(DevToken 노출)일 때만 가능
docker compose run --rm --no-deps \
  -e K6_PROMETHEUS_RW_SERVER_URL=http://<EC2_IP>:9090/api/v1/write \
  k6 run \
  -e BASE_URL=http://<EC2_IP>:8080 \
...
관측은 5장 쿼리를 그대로 쓰되 **EC2의 Grafana**(`http://<EC2_IP>:3000`)에서 본다.

> ⚠️ **부하 테스트 중에는 Grafana 대시보드를 열어두지 않는다.** 패널마다 5초 간격으로 쿼리가 나가 측정 대상의 CPU를 소모한다. 데이터는 TSDB에 쌓이므로 **테스트가 끝난 뒤** 열어 본다.

### 7.3 비용
비테스트 시간엔 AWS 리소스를 **반드시 중지**하는 **온디맨드 전제**로 운용한다(상시 기동 시 24/7 과금). 인스턴스 타입·금액·자동 중지 알람 설정은 [ADR 0005](adr/0005-aws-deployment-topology.md)가 SSOT다.

### 7.4 보안
부하 대상 공인 엔드포인트, DevToken을 노출하는 `prod,dev` 프로파일, 인증 없는 Prometheus 9090은 **측정 시간에만** 열고 끝나면 닫는다. 보안 그룹은 본인 공인 IP `/32`로 제한한다. 부하테스트용 DB·환경을 운영과 분리한다.
