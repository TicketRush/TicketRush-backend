 ...
 *
 * <p>소셜 로그인 요청의 socialId · socialProvider 은 {@code @JsonProperty} 로 이름을 적은 것이라 **그 이름이 곧 실제 계약**이다.
 * 전략이 여기까지 닿으면 문서가 실제 요청과 어긋나고, 문서를 믿은 클라이언트는 조용히 인증에 실패한다.
 *
...
class OpenApiSchemaNamingTest {
  ...
  void explicitlyNamedPropertiesAreKept() {
    ...
    Map<String, Schema> schemas =
        converters.readAll(
            com.ticketrush.boundedcontext.user.app.dto.request.SocialLoginRequest.class);
    Map<String, Schema> properties = schemas.get("SocialLoginRequest").getProperties();

    assertThat(properties)
        .containsKeys("socialId", "socialProvider")
        .doesNotContainKeys("social_id");
  }
}
