...
이 회차에서 **처음 확인한 함정**이다. 유입이 이미 끝난 회복 구간이라 적체는 단조 감소해야 하는데, Prometheus 로 뜬 값은 **내려가다 다시 올라간다.**

`timeseries-kafka-consumer-lag-main.json` (커밋됨, `max by (instance, topic)`, 15초):

```
booking-service:8090   6,392 → 5,812 → 5,232 → 4,632 → 4,052 → 6,073 ↑ → 5,473 → 4,893 → 4,293
ticket-service:8090    5,972 → 4,692 → 6,033 ↑ → 4,713 → 6,248 ↑ → 4,928 → 3,919 → 3,919 → 3,919
                                                                              └── 3표본 정체 ──┘
```

같은 구간을 브로커에서 5초로 뜬 `lag-samples-spike.csv` 는 **표본 47개에서 단조 감소 위반이 0회**다(정점 19,870 → 0).

| 축 | Prometheus 15초 (`records-lag`) | 브로커 5초 (`LEO − committed`) |
|---|---|---|
| 곡선 모양 | **톱니** — 회복 구간에서 재상승 booking 2회 · ticket 3회 | **단조 감소** (위반 0/47) |
| 정체 구간 | ticket 3,919 에서 3표본(45초) 고정 | 없음 |
| 회복시간·드레인율 산출 | **불가** | 가능 |

원인은 지표의 정의다. 이건 파티션별로 **"마지막 fetch 응답 시점의 lag"** 이고, `max.poll.records=20` 인 단일 스레드가 파티션 3개를 번갈아 훑으므로 **파티션마다 갱신 시각이 다르다.** 어떤 파티션의 값은 방금 갱신됐고 어떤 값은 수십 초 전 것이다 — 집계하면 신선한 값과 낡은 값이 섞이고, 낡은 파티션이 갱신되는 순간 값이 위로 튄다. ticket 의 3표본 정체는 그 파티션이 그동안 한 번도 fetch 되지 않았다는 뜻이다.
...
> 위 수치는 전부 커밋된 두 파일에서 재현된다. 회차 중 `sum by (instance)` 로도 같은 현상을 봤지만 그 쿼리의 덤프를 뜨기 전에 인스턴스가 내려가 원자료가 없어, **커밋된 `max by` 계열로만 논증한다.**
