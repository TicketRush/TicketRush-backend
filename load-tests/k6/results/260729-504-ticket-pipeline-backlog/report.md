...
| 쿼리 | 정점 |
|---|---|
| `sum by (instance, topic)` — 총 적체 | booking 19,870 / ticket 19,505 |
| `max by (instance, topic)` — 파티션 최댓값 | booking 6,508 / ticket 6,248 |

`6,508 × 3 = 19,524` 로 총합에 거의 일치한다. **세 파티션에 고르게 분산됐다.** 메시지 key 를 `bookingId` 로 두어 코호트 전체가 흩어졌기 때문이다.

> 대시보드의 `Kafka Consumer Lag` 패널과 런북 §10.3 은 `max by (...)` 를 쓴다. 그걸 총 적체로 읽으면 이 회차의 정점을 **1/3 로 과소평가**한다. §15.4 에 `sum` 을 명시해 뒀다.
