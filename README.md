...
> ℹ️ 세 검사는 **required status check로 걸려 있지 않습니다.** 즉 실패한 상태로도 머지가 물리적으로 막히지는 않으니, 통과를 확인하고 머지하는 것은 규율에 맡겨져 있습니다. 브랜치 보호가 실제로 강제하는 것은 **리뷰 대화 해결**(`required_conversation_resolution`)이며, approve 개수는 요구하지 않습니다.
