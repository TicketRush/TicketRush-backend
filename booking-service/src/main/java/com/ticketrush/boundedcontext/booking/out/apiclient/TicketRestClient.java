...
public class TicketRestClient {
   ...
   * BOOKING_TICKET_COMMUNICATION_FAILED}(503)로 수렴시켜 좌석을 SOLD로 남기는 안전한 방향(취소 거부)으로 전파한다. 이 클래스에서
   ...
   * </ul>
   *
   * <p>알 수 없는 상태 문자열도 막는다. ticket-service가 {@code TicketStatus}에 값을 추가하거나 이름을 바꿔도 booking은 컴파일 에러
