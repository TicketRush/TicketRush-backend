 ...
 *
 * <p>{@code refundAllowed} 는 환불 마감(D-7) 판정이다 (#668). <b>{@code withRefundDeadline=true} 일 때만
 * 채워진다</b> — 판정에 performance-service 왕복이 필요해서, 기본값으로 켜면 입장 검증(ticket-service)과 결제 확정 결로에까지 그 왕복이
 * 얹힌다. 그 둘은 마감을 필요로 하지 않고 지연에 민감한 경로다.
 */
...
public record BookingInternalResponse(
    ...
    @Schema(
            description = "환불 마감(D-7) 이 허용하는가. withRefundDeadline=true 일 때만 채워지고, 그 밖에는 null 이다.",
            example = "true")
  ...
  /** 마감 판정을 얹어 돌려준다 (#668). 조회 트랜잭션 밖에서 공연을 조회해야 하므로 별도 단계로 둠. */
  public BookingInternalResponse withRefundAllowed(boolean allowed) {
    return new BookingInternalResponse(bookingId, userId, bookingStatus, bookingNumber, allowed);
  }
}
