...
## 커밋 후 기존 이슈/PR 본문 동기화 (필수)

- **커밋을 한 뒤에는**, 현재 브랜치(`feature/{번호}`)에 연결된 **이미 발행된 이슈·PR이 있는지 확인**하고, 있으면 그 **본문을 이번에 실제로 한 작업까지 반영하도록 업데이트**한다. (이슈/PR 본문이 작업과 어긋난 채로 남지 않도록 유지)
...
- **반영 원칙:**
  ...
  - **코드는 수정하지 않는다.** 이슈/PR 본문 텍스트만 다룬다.
- 본문 갱신 시 임시 파일은 Bash 힙독으로 만들고, 팀 컨벤션(`docs/backend-convention.md` 단일 출처) 용어를 쓴다.
