...
- `$ARGUMENTS` (선택) = 모듈명/`all`/테스트 클래스 힌트. 예: `/test seat-service`, `/test all`,
  `/test seat-service --tests "*SeatHoldTest"`.
...
- **커밋·push 하지 않는다.**
- 모듈 목록은 **`settings.gradle`을 신뢰**한다(CLAUDE.md 목록이 아님).
...
- 각 경로의 **첫 세그먼트 = 모듈명** 후보 → `settings.gradle`에 `include`된 모듈명과 **교집합**만 채택(`.claude`, `docs`,
  `.github`, `config`, `gradle` 등 비모듈 제외).
- **`common` 변경이 포함되면 전체 테스트로 승격**한다. (common은 다른 모듈이 의존 + test-fixtures 제공 → 영향 범위 넓음.) 그 이유를 한 줄로
  보고한다.
...
- **실패 시**: 실패한 테스트명과 핵심 메시지만 추출한다 (필요하면 `<모듈>/build/test-results/test/*.xml` 또는 콘솔의 `FAILED` 라인). *
  *전체 로그를 그대로 붙이지 않는다.**
