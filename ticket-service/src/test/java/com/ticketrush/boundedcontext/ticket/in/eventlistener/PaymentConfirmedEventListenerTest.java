...
class PaymentConfirmedEventListenerTest {
  ...
  @Test
  @DisplayName("최초 수신이면 해당 bookingId로 티켓을 발급하고 오프셋을 커밋한다")
  ...
  @Test
  @DisplayName("이미 처리된 이벤트(inbox 중복)면 발급을 실행하지 않고 오프셋만 커밋한다")
  void handlePaymentConfirmedSkipsDuplicate() {
  ...
  void handlePaymentConfirmedAcksOnInboxRace() {
    // given
    final DomainEventEnvelope envelope = envelope();
    given(jsonConverter.deserialize(PAYLOAD, PaymentConfirmedEvent.class)).willReturn(event());
    given(
