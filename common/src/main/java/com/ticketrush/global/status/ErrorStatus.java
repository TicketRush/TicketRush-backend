...
public enum ErrorStatus {
  ...
  /*
   * 관리자 강제 해제 전용 3종(#562). 해제 경로들(만료 리스너·스케줄러·즉시 취소)이 비HOLD 좌석을 조용한
   * no-op으로 넘기는 것과 갈린다 — 그쪽은 호출자가 이미 결과를 확정한 뒤라 되돌릴 것이 없지만, 관리자는
   * 버튼을 누른 결과를 화면에서 알아야 한다.
   *
   * 셋을 하나로 뭉개지 않는 이유는 화면이 안내할 다음 행동이 서로 다르기 때문이다 — 판매 완료는 환불로
   * 가야 하고, 경합은 재조회 후 재시도가 유효하며, 이미 해제된 좌석은 할 일이 없다. SEAT_409_003을 따로
   * 둔 것과 같은 논리다.
   */
  SEAT_NOT_HELD(HttpStatus.CONFLICT, "SEAT_409_004", "선점 중인 좌석이 아니어서 강제 해제할 수 없습니다."),
  SEAT_SOLD_NOT_RELEASABLE(
      HttpStatus.CONFLICT, "SEAT_409_005", "판매 완료된 좌석은 강제 해제할 수 없습니다. 환불 처리로 반환해 주세요."),
  SEAT_RELEASE_CONFLICT(
      HttpStatus.CONFLICT, "SEAT_409_006", "요청 처리 중 좌석 선점 상태가 바뀌어 해제하지 못했습니다. 다시 조회한 뒤 시도해 주세요."),
